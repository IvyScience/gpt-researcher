...
class BaseInternalBiblioRetriever:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.base_url = (
            self.headers.get("internal_api_base_url")
            or os.getenv("INTERNAL_API_BASE_URL", "https://www.ivysci.com")
        )

        # Get API key from headers or environment variable
        self.api_key = (
            self.headers.get("internal_api_key")
    ...
    def _transform_item(self, item: Any) -> Optional[Dict[str, Any]]:
        ...
        if doi:
            ...
            href = (
                ...
                or item.get("source")
                or item.get("link")
                or metadata.get("biblio_id")
                or metadata.get("highlight_id")
        ...
        title = str(structured.get("title") or metadata.get("title") or "").strip()
        year = str(structured.get("year") or metadata.get("year") or "").strip()
        doi_for_note = str(structured.get("doi") or "").strip()

        # authors: [{"name": "A"}, {"name": "B"}, ...]
        raw = structured.get("authors") or metadata.get("authors") or []
        names = [a.get("name") or a.get("family", "") for a in raw[:3] if isinstance(a, dict)]
        names = [str(n).strip() for n in names if n]
        if len(names) == 2:
            author = f"{names[0]} & {names[1]}"
        elif len(names) > 2:
            author = f"{names[0]} et al."
        else:
